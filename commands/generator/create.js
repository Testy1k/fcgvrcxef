...
module.exports = {
  ...
  usage: 'create <free/basic/boost/premium> <servicename>',
    ...
    execute: async (message, args, usedPrefix) => {
      // Check if the user has the restock role
        ...
        if (args.length < 1 || args.length > 2) {
            return message.reply('Please provide the correct arguments. Usage: `create (free/prem/boost/basic) [fileName]`.');
        }

        const keyword = args[0].toLowerCase();

        if (['free', 'premium', 'boost', 'basic'].includes(keyword)) {
            try {
...
function generateRandomContent() {
    const lines = 10000; // You can adjust this number based on your preference
    let content = '';
